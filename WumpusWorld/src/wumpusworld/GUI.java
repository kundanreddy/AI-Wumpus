    ...
    private void updateGame()
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                {
                    if (w.getDirection() == World.DIRECTION_DOWN) blocks[i][j].add(new JLabel(l_player_down));
                    if (w.getDirection() == World.DIRECTION_UP) blocks[i][j].add(new JLabel(l_player_up));
                    if (w.getDirection() == World.DIRECTION_LEFT) blocks[i][j].add(new JLabel(l_player_left));
                    if (w.getDirection() == World.DIRECTION_RIGHT) blocks[i][j].add(new JLabel(l_player_right));
                }
